...
def lambda_handler(event, context):
    ...
    Required Lambda function environment variables are the following:
        ...
        - SECRET_ARNS: List of ARNs of secrets created in Secrets Manager
        - USER_NAMES: List of Usernames of ElastiCache user
    ...
    step = event['Step']
    env_secret_arn = os.environ['SECRET_ARNS'].split(",")
    if secret_arn not in env_secret_arn:
    ...
    logger.info("secret_arn : {}".format(secret_arn))
...
def verify_user_name(secret):
    ...
    '''
    env_elasticache_user_name = os.environ['USER_NAMES'].split(",")
    secret_user_name = secret["username"]
    if secret_user_name not in env_elasticache_user_name:
